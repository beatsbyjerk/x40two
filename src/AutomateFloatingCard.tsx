  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-app-primary-30">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2 py-1 bg-app-tertiary border border-app-primary-40 rounded color-primary font-mono text-xs hover:bg-app-secondary hover:border-app-primary transition-colors"
              title="Reset Volume"
            >
              Reset
            </button>
        ...
        {/* Latest Trending Ticker Bar */}
        <div className="bg-app-primary-60 border-b border-app-primary-30 px-4 py-2">
          <div 
            className="flex items-center justify-between gap-4 cursor-pointer hover:bg-app-primary-40 transition-colors p-2 rounded"
            onClick={() => setShowUSD(!showUSD)}
            title="Click to toggle between SOL and USD"
          >
            <div className="flex items-center gap-1">
              <span className="text-app-secondary-60 font-mono text-xs">TRENDING:</span>
            </div>
            
            {/* Buy Volume */}
            <div className="flex items-center gap-1">
              <span className="text-app-accent font-mono font-semibold text-xs">BUY:</span>
              <span className="color-primary font-mono text-xs">{formatVolumeDisplay(cumulativeBuyVolume)}</span>
            </div>
            
            {/* Sell Volume */}
            <div className="flex items-center gap-1">
              <span className="text-error-alt font-mono font-semibold text-xs">SELL:</span>
              <span className="text-error-alt font-mono text-xs">{formatVolumeDisplay(cumulativeSellVolume)}</span>
            </div>
            
            {/* Net Volume */}
            <div className="flex items-center gap-1">
              <span className="text-app-secondary-60 font-mono font-semibold text-xs">NET:</span>
            ...
            {(() => {
              ...
              if (!latestTrade) {
                return (
                  <div className="flex items-center gap-1">
                    <span className="text-app-secondary-60 font-mono text-xs">NO TRADES</span>
                  </div>
                );
              }
              ...
              return (
                <>
                  <div className="flex items-center gap-1">
                    <span className={`px-1 py-0.5 rounded text-xs font-mono font-semibold ${
                      latestTrade.type === 'buy' ? 'bg-app-accent text-black' : 'bg-error-alt text-white'
                    }`}>
                      {latestTrade.type.toUpperCase()}
                    </span>
                    <span className="color-primary font-mono text-xs">{formatVolumeDisplay(latestTrade.solAmount)}</span>
                  </div>
                  
                  <div className="flex items-center gap-1">
                    <span className="color-primary font-mono text-xs">{formatPrice(latestTrade.avgPrice)}</span>
                  </div>
                  
                  <div className="flex items-center gap-1">
                    <span className="color-primary font-mono text-xs">{formatLargeNumber(latestTrade.marketCap)}</span>
                  </div>
                  
                  <div className="flex items-center gap-1">
                    <span className="color-primary font-mono text-xs">{formatAddress(latestTrade.address)}</span>
                  </div>
                </>
              );
            })()}
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="flex-1 p-4 overflow-y-auto">
              {activeTab === 'data' && (
                <div className="mb-6">
                  <h4 className="text-lg font-mono font-semibold color-primary mb-4">Market Data</h4>
                  {/* Other market data content can go here */}
                </div>
              )}
