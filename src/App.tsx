...
const DocsPage = lazy(() => import('./x40two_docs_page').then(module => ({ default: module.default })));
...
const WalletManager: React.FC = () => {
  ...
  interface AppState {
    ...
    config: ConfigType;
    currentPage: 'wallets' | 'chart' | 'actions' | 'docs';
    wallets: WalletType[];
  ...
  type AppAction = 
    ...
    | { type: 'SET_CURRENT_PAGE'; payload: 'wallets' | 'chart' | 'actions' | 'docs' }
  ...
  const memoizedCallbacks = useMemo(() => ({
    ...
    setCurrentPage: (page: 'wallets' | 'chart' | 'actions' | 'docs') => dispatch({ type: 'SET_CURRENT_PAGE', payload: page }),
  ...
  useEffect(() => {
    ...
    const handleNavigateToDocs = () => {
      memoizedCallbacks.setCurrentPage('docs');
      window.history.pushState({}, '', '/docs');
    };

    window.addEventListener('openSettingsWalletsTab', handleOpenSettingsWalletsTab);
    window.addEventListener('navigateToDocs', handleNavigateToDocs);

    return () => {
      window.removeEventListener('openSettingsWalletsTab', handleOpenSettingsWalletsTab);
      window.removeEventListener('navigateToDocs', handleNavigateToDocs);
    };
  }, []);
  ...
  useEffect(() => {
    const handlePopState = () => {
      ...
      if (path === '/docs') {
        ...
        memoizedCallbacks.setCurrentPage('wallets');
      }
    };
